...
const getUsers = async (req, res) => {
    try {
        if (req.user && req.user.type === 'editor') {
            return res.status(403).json({ message: 'Access denied. Editors are not authorized to view users.' });
        }
...
const loginUser = async (req, res) => {
    try {
        ...
        // Enhancement Check: Block accounts with a 'viewer' role from logging in
        if (user.type === 'viewer') {
